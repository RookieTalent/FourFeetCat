...
> 本文档定义 FourFeetCat 项目的功能需求和非功能需求，作为后续技术方案设计、研发实施、测试验收的依据。本文档回答 **What**，不回答 **How**，How 在后续的技术方案中展开。前置阅读《项目篇 FourFeetCat 业界调研》，本文档基于调研得出的领域判断，不重复论证企业 Agent OS 领域的现状。
...
FourFeetCat 是基于 Java 实现的面向企业场景的 **Agent OS**。它装在企业自己的 K8s 或服务器上，作为统一底座，在底座上跑各种业务 Agent（运维助手、客服助手、HR 助手、销售助手、知识管理助手等），共享一套渠道接入、模型路由、工具调用、记忆系统、沙箱执行能力。数据完全留在企业自己的基础设施，不锁任何云生态。

业界已经有开源 Agent 项目把这套设计验证过（OpenClaw 用 Node.js，Hermes Agent 用 Python），但 Java 生态没有任何项目把"Agent OS"作为定位。Java 是大量企业现有后端的事实标准技术栈，Spring AI Alibaba 已经把底层 LLM 调用解决了，缺的就是上面那一层"Agent OS"。FourFeetCat 填这个位置。
...
理解这个分层，才能看懂 FourFeetCat 的交付节奏：

1. **核心阶段**：先把 Agent OS 的运行时内核用 Java 做扎实，这一层在能力上对齐业界开源 Agent OS 的基础层
2. **扩展阶段**：FourFeetCat 真正的差异化治理层（多租户、SSO、完整审计、Tool 治理），在核心内核之上由扩展阶段和社区共建陆续补齐

核心阶段交付的是 Agent OS 的**内核底座**，而不是一个治理能力完备的企业级 Agent OS，后者是终局，核心阶段是地基。

---

### 1.2 FourFeetCat 能干什么

FourFeetCat 优先做五个核心能力，基于这五个能力可以扩展出企业里大量真实需求。这五个能力都属于"让单个 Agent 跑得好"的运行时内核层；让 FourFeetCat 成为真正"OS"的多 Agent 治理能力（多租户、Tool Policy、审计、SSO），在扩展和社区阶段补齐。

#### 能力一：对接 LLM

FourFeetCat 通过 Provider 抽象层对接主流大模型（DeepSeek、通义、Kimi、智谱、混元、豆包、Anthropic、OpenAI 等），Agent 不感知具体调的是哪家模型，运行时切换无 lock-in。
...
Agent 能调用工具实际操作系统。FourFeetCat 提供两类 Tool：

- **内置 Tool**：FourFeetCat 自带的基础工具（读写文件、执行 Shell、发起 HTTP 请求）
...
FourFeetCat 通过完整的 REST API 把所有能力对外暴露，业务系统用 HTTP 调一下就能用上 Agent，不用关心内部怎么实现。Web Service 是 FourFeetCat 的对外门面，是企业把 AI 能力嵌入已有业务系统的唯一通道。
...
| 术语 | 定义 |
|------|------|
| **Agent（智能体）** | 一个具象的业务智能体，定义本体是一个目录 `.fourfeetcat/agents/<name>/`：`AGENT.md`（frontmatter 是运行配置、正文是任务指令）加可选 `skills/` 公共 Skill 软连接、`scripts/` 脚本、`REFERENCE.md` 参考。一个目录就是一个完整可用的业务 Agent，不是写代码写出来的 |
...
| **Tool（工具）** | Agent 可以调用的外部能力。内置 Tool 是 FourFeetCat 自带的（文件、Shell、HTTP、通知推送）；Plugin Tool 是业务方自己写的 |
| **Memory（记忆）** | Agent 跨对话保留的状态，分三层：会话记忆、长期记忆（MEMORY.md）、情景记忆（扩展阶段） |
| **Channel（渠道）** | Agent 对外接入的消息入口，包括 CLI、企业微信、飞书、钉钉、Slack 等 |
| **Web Service** | FourFeetCat 对外暴露的完整 REST API，是业务系统集成 FourFeetCat 的唯一通道 |
...
| **Skill（技能）** | 公共实体存 `.fourfeetcat/skills/<name>/`，Agent 通过自身 `skills/<name>` 相对软连接选择可见集合。`ContextLoader` 每轮只注入已绑定 Skill 的 name、description 和本地读取路径；正文与附属资源经 `read_file`/`shell` 按需进入上下文。Skill 不是 Tool，不进 `ToolRegistry` |
| **Bootstrap（引导文件）** | 加载到系统提示词中的上下文文件：AGENTS.md（项目级 agent 行为说明）、SOUL.md（agent 人格定义）、USER.md（用户偏好） |
| **Workspace（工作区）** | FourFeetCat 实例的工作目录，默认是 `.fourfeetcat/`，包含 Agent 目录、全局 Skill 库、Bootstrap 文件、记忆、会话、产出物、日志的子目录 |

---

## 3. 设计目标

FourFeetCat 的核心目标可以用四个词概括：**统一、私有、易接入、可观测**。

| 目标 | 说明 |
|------|------|
| **统一** | 企业内多个业务 Agent 共享同一套底座。Channel、Provider、Tool、Memory、Sandbox 这些公共能力下沉到 FourFeetCat，企业上一个新 Agent 放一个 Agent 目录（一份 `AGENT.md`）就能跑起来 |
| **私有** | 数据完全留在企业自己的基础设施上，部署在企业自己的 K8s、虚拟机或物理机上。FourFeetCat 本身不收集任何企业数据 |
...
以下三个典型场景描述 FourFeetCat 完整形态（含扩展阶段能力）下的目标用法，核心阶段先具备其运行时内核。

### 场景一：运维助手

某中型 SaaS 公司的运维团队基于 FourFeetCat 搭一个运维助手，接入企业微信。Agent 配了几个 Tool（告警分诊、日志查询、服务重启、变更审批）。凌晨告警通过 webhook 进 FourFeetCat，Agent 收到告警后调用日志查询 Tool 拉错误堆栈，跟历史故障库交叉引用发现是已知 bug，自动应用 mitigation Skill 重启服务，在企业微信运维群里汇报"已自愈，详情见附件"，值班工程师早晨起来看下记录就行。

**FourFeetCat 在此场景的角色**：Channel 接入（企业微信）、Provider 路由（主备 LLM）、Tool 调用（SSH、Prometheus、Slack 通知）、Memory（历史故障库）、Skill（自愈 runbook）。

### 场景二：知识管理助手

某金融企业的法务团队基于 FourFeetCat 搭一个知识管理 Agent，接入飞书。Agent 索引了内部的合同模板、法规文档、历史案例、咨询记录。员工在飞书里问"上次签 SaaS 服务协议是怎么处理数据出境条款的"，Agent 检索 Memory 拉出历史案例，综合相关法规给出建议草稿，标注引用来源。

**关键点**：Memory 检索准确度和引用追溯（合规要求所有 Agent 回复必须可追溯到引用源）。

### 场景三：销售助手

某制造业企业的销售部门基于 FourFeetCat 搭一个客户洞察 Agent，接入企业微信和 CRM。销售跑客户前问 Agent"明天去拜访 A 公司，有什么我需要知道的"，Agent 调用 CRM connector 拉客户历史交易记录，调用企查查 MCP 工具查最新工商信息，调用知识库 Tool 提取关键决策人和采购习惯，综合输出客户简报。

**FourFeetCat 在此场景的核心能力**：MCP 集成（外部数据）、企业 IT 系统 connector（自家 CRM）、Tool 编排。
...
FourFeetCat 的工作目录是 `.fourfeetcat/`，通过 `fourfeetcat init` 命令初始化。

```bash
fourfeetcat init   # 在当前目录下创建 .fourfeetcat/ 工作区
```
...
- `fourfeetcat init` 幂等：已存在的目录和文件一律不覆盖。六个子目录建好后，用 `fourfeetcat profile create <name>` 生成第一个 Agent 目录

---

### 5.2 定义一个 Agent：AGENT.md

一个目录 = 一个 Agent。`.fourfeetcat/agents/<name>/AGENT.md` 由两部分组成：**frontmatter** 是这个 Agent 自己的 profile（用哪个 Provider/模型、能用哪些 Tool、绑定哪个 Channel、要不要定时），**正文**是任务指令。`AgentLoader.deriveProfile()` 把 frontmatter 派生成底座认识的 `Profile`。Agent 的 Skill 绑定不写进 frontmatter，而由同目录 `skills/` 下的相对软连接表达。
...
**Agent 管理命令**（命令组名沿用 `profile`，操作的是 `.fourfeetcat/agents/` 下的 Agent 目录）：

```bash
fourfeetcat profile create <name>    # 创建 Agent（生成最小 AGENT.md 模板）
fourfeetcat profile list             # 列出全部 Agent
fourfeetcat profile show <name>      # 查看某个 Agent 的 AGENT.md
fourfeetcat profile delete <name>    # 删除 Agent（整个目录）
```

核心阶段支持创建并管理多个 Agent，多个 Agent 可以在同一个 FourFeetCat 实例上并存，这是"OS"在核心阶段的最小体现。
...
核心阶段直接基于 Spring AI Alibaba 的 `ChatClient` 实现。Spring AI Alibaba 已经做好了主流 LLM（DeepSeek、通义、文心、Kimi、智谱、混元、豆包、Anthropic、OpenAI 等）的 connector，FourFeetCat 把它们包装成 Provider，不重复造轮子。
...
ReAct 循环是 Agent 的核心工作机制，也是 FourFeetCat 最关键的一段代码。
...
- 存在 `.fourfeetcat/memory/MEMORY.md` 一个 Markdown 文件，跨所有对话保留
...
**用户核心体验**：用 FourFeetCat 一段时间后，Agent 自然会记住用户的偏好、项目信息、关键决策，下一次对话不需要重新解释。这是 Agent OS 区别于 chatbot 的核心体验。

---

### 5.6 Tool 体系（核心能力四：让 Agent 能干事）

Tool 是 Agent 可以调用的外部能力。Agent 通过 LLM Function Calling 决定何时调哪个 Tool，FourFeetCat 负责 Tool 的注册、查找、调用、结果回传。
...
1. 建 `.fourfeetcat/agents/daily-pr-digest/`，写一份 `AGENT.md`：frontmatter 声明 provider、`mcp_servers`、`schedules`，正文写任务指令
...
核心阶段只内置一种 Channel：**CLI Channel**，通过 `fourfeetcat chat` 命令启动，支持多轮对话、查看上下文、查看 Tool 调用记录。
...
Web Service 是 FourFeetCat 的对外完整门面，业务系统通过 REST API 接入 FourFeetCat 的所有能力。这是 FourFeetCat 区别于偏个人定位的 OpenClaw、Hermes 的关键能力。
...
核心阶段 Session 数据持久化到本地 SQLite（`.fourfeetcat/sessions/` 下）。重启 FourFeetCat 后，正在进行的 Session 可以恢复。Session 上下文超过 LLM 的 context window 时，简单截断早期对话保留近期对话。
...
| 模式 | 命令 | 说明 |
|------|------|------|
| 交互对话 | `fourfeetcat chat` | 交互式多轮对话，开发调试和日常使用的主要方式。`--message "xxx"` 可发单条消息后退出 |
| HTTP API | `fourfeetcat serve` | 启动后在指定端口（默认 8080）开放 RESTful 接口，业务系统通过 HTTP 调用 |
| 守护进程 | `fourfeetcat gateway` | 常驻守护进程，同时服务多个 Channel |
...
| 类别 | 命令 | 说明 |
|------|------|------|
| 启动和状态 | `fourfeetcat init` | 初始化工作区 |
| 启动和状态 | `fourfeetcat status` | 查看配置和运行状态 |
| 启动和状态 | `fourfeetcat chat [--profile <name>]` | 交互对话 |
| 启动和状态 | `fourfeetcat serve` | 启动 HTTP API 服务 |
| 启动和状态 | `fourfeetcat gateway` | 启动多渠道守护进程 |
| Profile 管理 | `fourfeetcat profile list` | 列出所有 Profile |
| Profile 管理 | `fourfeetcat profile create <name>` | 创建新 Profile |
| Profile 管理 | `fourfeetcat profile show <name>` | 查看 Profile 详情 |
| Profile 管理 | `fourfeetcat profile delete <name>` | 删除 Profile |
| 查询 | `fourfeetcat provider list` | 列出已配置的 Provider |
| 查询 | `fourfeetcat tool list` | 列出已注册的 Tool |
| 查询 | `fourfeetcat session list` | 列出会话历史 |
...
FourFeetCat 作为开源项目，需要一个独立的主页作为对外门面，讲清楚 FourFeetCat 是什么、能干嘛、怎么用，引导开发者快速上手。

主页在核心阶段做出来，与核心代码同期发布，作为 FourFeetCat 1.0 对外亮相的一部分。技术栈推荐使用 VitePress、Astro 或 Docusaurus 等静态站点生成器。
...
- **MCP Server 暴露**：FourFeetCat 自己作为 MCP server，把内部 Agent 能力暴露给其他系统
...
> 这一层是 FourFeetCat 区别于个人级 Agent OS 的核心差异化所在
...
社区共建功能不在 FourFeetCat 主线开发计划内，作为长期方向开放给社区贡献，不规定时间表。
...
- **多区域部署**：跨地域 FourFeetCat 集群，Agent、Memory、Session 可以跨区域协同
...
| FourFeetCat 内部转发开销 | ≤ 50ms |

（LLM 调用本身的延迟取决于 Provider，不在 FourFeetCat 控制范围内）
...
- **LLM 协议**：OpenAI 兼容协议是事实标准，只要 Provider 实现这套协议，FourFeetCat 就能直接接
...
- **数据驻留**：FourFeetCat 不主动外发任何数据，所有数据留在企业自己的基础设施上
...
```
用户执行 fourfeetcat init
  → FourFeetCat 创建 .fourfeetcat/ 目录及六个子目录
...
```
用户执行 fourfeetcat profile create <name>
  → FourFeetCat 在 .fourfeetcat/agents/<name>/ 下创建 AGENT.md（最小模板）
用户编辑 AGENT.md：frontmatter 配 Provider、Tool 列表、Channel、引用的 Skill，正文写任务指令
用户执行 fourfeetcat chat --profile <name>
...
消息从 Channel 进来（CLI 输入或 HTTP API 调用）
  ...
  → 调用 LLM Provider 获取响应
  → [有 Tool 调用] → FourFeetCat 执行 Tool → 结果回传给 LLM 继续生成
...
LLM 通过 Function Calling 指明 Tool 名称和参数
  → FourFeetCat 从 Agent 工具池找到对应 Tool
  → 做参数校验和白名单校验
  → 内置 Tool：在 FourFeetCat 进程内执行（白名单约束下）
...
```
用户第一次跟 Agent 说话
  → FourFeetCat 用 Channel+用户+Agent 联合 ID 查活跃 Session
...
长期记忆是 `.fourfeetcat/memory/MEMORY.md` 一个 Markdown 文件，按追加方式写入，无结构化 schema。扩展阶段引入向量库后，Memory 才有结构化的 embedding 存储。
...
FourFeetCat 核心功能的实施按 **4 周节奏**组织，每周 3 小时，合计 12 小时。

| 周次 | 时间投入 | 能力主线 | 可演示成果 |
|------|---------|---------|-----------|
| **第一周** | 3 小时 | 对接 LLM + ReAct 循环（核心能力一+二） | `fourfeetcat chat` 多轮对话，Agent 通过 ReAct 调 HTTP Tool 完成天气查询 |
| **第二周** | 3 小时 | Memory + Tool 体系（核心能力三+四） | Agent 记住用户偏好并在后续对话用到，能调本地文件和外部 MCP server |
| **第三周** | 3 小时 | Web Service（核心能力五） | 外部系统通过 10 个 REST 端点完整调用 FourFeetCat |
| **第四周** | 3 小时 | 多 Agent 演示 + 工程化收尾 | 多 Agent 并存可用，CLI 完整，Session 跨重启恢复，项目主页可访问 |

### 各周详细实施内容

**第一周**（3 小时）：对接 LLM + ReAct 循环
- `fourfeetcat init` 工作区初始化、`AGENT.md` frontmatter 解析
...
- 通过 `fourfeetcat serve` 启动 Spring MVC 服务
- 配置与密钥加载（环境变量注入 + 基础校验）
...
| **Tool 执行安全风险** | 应用层白名单不是完整 Sandbox，可能影响 FourFeetCat 进程 | 严格限制内置 Tool 能力范围，核心阶段不建议在生产环境跑高敏感场景 |
...
| **生态关系风险** | FourFeetCat 和 OpenClaw、Hermes 的关系 | 通过 markdown + frontmatter 的目录形态互通，生态互补不竞争；OpenClaw 偏个人、Hermes 偏小团队、FourFeetCat 定位企业 |

### 未决事项

| 事项 | 说明 | 决议时间 |
|------|------|---------|
| Provider 抽象接口设计 | 直接用 Spring AI `ChatClient`，还是在 `ChatClient` 之上加一层 FourFeetCat 自己的抽象 | 技术方案阶段 |
...
- 命令行工具有清晰的帮助和错误提示
- 项目主页可访问，讲清楚 FourFeetCat 是什么、怎么快速开始
...
两个 Demo 都是"钟推"（`AgentScheduler` 到点自动触发），但都要能同时支持"人推"手动补跑一次做验证（`fourfeetcat chat` 或 `POST /agents/{name}/invoke`），验证同一个 Agent 不管从哪个入口触发，走的都是同一条 `AgentService` 链路。

---

## 14. 总结

FourFeetCat 是基于 Java 实现的面向企业场景的 Agent OS，装在企业自己的 K8s 或服务器上，作为统一底座跑各种业务 Agent，共享一套渠道接入、模型路由、工具调用、记忆系统、沙箱执行能力。

**FourFeetCat 的交付分两段：**

1. 核心阶段先用 Java 把 Agent OS 的运行时内核做扎实，这一层在能力上对齐业界开源 Agent OS 的基础层
2. FourFeetCat 真正的差异化治理层（多租户、SSO、完整审计、Tool 治理），在核心内核之上由扩展阶段和社区共建陆续补齐。**核心阶段是地基，企业级治理是终局。**
...
**核心理念**：FourFeetCat 五大能力扎实落地，业务方组合 Agent 目录 + MCP server 就能解决业务问题，通过 Web Service 接入已有系统，不需要写 Agent 后端代码。FourFeetCat 不绑定具体业务，业务方按自己的需求组合。
