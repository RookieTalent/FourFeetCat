<!-- 来源讲义：第18节《CLI 功能概述、实现思路与代码讲解》。本文件由该 PDF 逐页照搬整理为 Markdown，正文未改动内容，仅按原文版式重排；代码仅在 PDF 换行处还原为完整语句。 -->
...
```text
CLI 入口（本节）                        Web Service 入口
终端交互、调试　chat / serve / gateway      业务系统走 REST API 集成
              ↓                ↓
             ReAct 引擎（中枢）
             接消息 → 想 → 调工具 → 回结果
              ↓        ↓        ↓
