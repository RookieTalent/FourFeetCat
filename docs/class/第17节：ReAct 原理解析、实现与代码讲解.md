<!-- 来源讲义：第17节《ReAct 原理解析、实现与代码讲解》。本文件由该 PDF 逐页照搬整理为 Markdown，正文未改动内容，仅按原文版式重排；代码仅在 PDF 换行处还原为完整语句。 -->
...
```text
Reason 想          Act 做          Observe 看
想下一步做什么      调一个工具执行    拿结果，并回上下文

还没完成 → 带着新状态回到 Reason
这轮没有工具调用 → 返回最终响应 ｜ 转够最大轮数（默认 10）→ 强制结束
```
...
↓
组装 Prompt
PromptBuilder：system + Memory + 历史 + 工具
↓
调大模型（ProviderService）
↓
有工具调用？
├─ 有 → ToolExecutor 执行 → 结果回填 → 回到组装 Prompt
└─ 没有 → 返回最终响应

每轮把模型响应和工具结果都追加回 Session · 转够最大轮数强制收尾
```
