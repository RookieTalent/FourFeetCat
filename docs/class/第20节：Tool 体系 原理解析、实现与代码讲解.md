<!-- 来源讲义：第20节《Tool 体系 原理解析、实现与代码讲解》。本文件由该 PDF 逐页照搬整理为 Markdown，正文未改动内容，仅按原文版式重排；代码仅在 PDF 换行处还原为完整语句。 -->
...
```text
[LLM（想）]         --要调-->      [OryxOS（执行）]      --操作-->      [外部世界]
决定调哪个工具、传什么参数      找到工具、跑它、拿结果       文件 / Shell / HTTP / 外部系统
...
```text
方式一 · 零代码（主推）
写一个 AGENT.md 目录 描述意图 + 复用现成的 MCP server
方式二 · 轻代码
用任何语言自己写一个 MCP server，接企业自有系统
方式三 · 重代码
用 @Tool 注解写 Java Spring Bean，做深度集成
门槛 / 工程量：低 → 高
```
...
关键就两条：注册时把每个 MCP 工具包装成 OryxTool 塞进同一个 ToolRegistry （ReAct 循环由此对来源无感知）；执行时通过 MCP 协议原样转发、结果包成 ToolResult 。失联的 server 记 WARN 跳过，不能变成自己的启动故障。
...
看它在干嘛：@Tool 注解让 Spring AI 自动把这个方法注册成工具、并根据参数生成 schema；方法体里第一件事就是过安全校验，域名不在白名单里直接抛异常、请求根本发不出去；过了校验才真正去请求。安全校验写在执行的第一步，这是硬规矩——这里先用一下这个校验，具体 Sandbox / SandboxAction 怎么设计，23、24 节详细展开。
...
```text
内置 @Tool  \
业务 @Tool   →  ToolRegistry   →   ToolExecutor 一次调用
MCP 工具    /   统一成 OryxTool
  ① 按名字从 Registry 找到工具
  ② 白名单校验（23、24 节详讲）
  ③ OryxTool.execute(参数)
  ④ 包成 ToolResult + 写 tool_invocations
  校验不过 → 抛异常，执行终止
  结果回给 ReAct 循环，进下一轮
```
...
- 方式一（零代码）：在 .oryxos/skills/ 写一份 SKILL.md，Profile 里 skills 引用它、mcp_servers 引用要复用的 MCP server。
...
规划的九个内置工具覆盖了"读写文件、跑命令、调 API、记事、往外推"的最短链路。但对着 Claude Code、Cursor、OpenAI Assistants 这些主流 Agent 产品的公约数一比，会发现少了几样日常任务里高频、缺了就明显笨手笨脚的工具。所以实现时又补了五个——它们不是新概念，全都复用本节已立好的 OryxTool 抽象、@Tool 注解管道和 Sandbox 检查位，正好也是对前面那套设计的一次实战检验：加工具确实只是"多写一个方法 / 多挂一个实现类"。

### 6.1 缺哪几样，为什么必须补

| 工具 | 干什么 | 不补的话 |
| --- | --- | --- |
| edit_file | 把文件里一段唯一出现的旧文本换成新文本 | 只有整文件覆盖写（write_file ），改一行配置要把整个文件重写一遍——既费 token 又容易把别的内容冲掉 |
