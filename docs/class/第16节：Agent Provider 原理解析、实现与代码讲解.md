<!-- 来源讲义：第16节《Agent Provider 原理解析、实现与代码讲解》。本文件由该 PDF 逐页照搬整理为 Markdown，正文未改动内容，仅按原文版式重排；代码仅在 PDF 换行处还原为完整语句。 -->
...
上层 · ReAct 循环
↓ 传入 Profile + Prompt
ProviderService（前台 / 统一抽象层）
按 Profile 选对应模型 · 屏蔽各家差异
↓
ChatModel · DeepSeek
ChatModel · Kimi
ChatModel · Qwen
↓
各家 LLM API（OpenAI / Anthropic / Gemini 协议）
```
...
```text
chat(...)
ReActLoop   ProviderService   ChatModel

① 从映射表取出 profile.provider 对应的 ChatModel
② 组装消息 + 工具 schema（关掉自动执行）
调用
发起调用
Response
③ 写 llm_calls（provider / model / token，成功失败都写）
返回 Response
（响应里可能带 tool 调用请求）
```
...
public Response chat(String sessionId, Profile profile, Prompt prompt) {
    ...
    try {
        ...
        audit.record(sessionId, profile.getProvider(), profile.getModel(),
                     resp.usage(),
...
- audit.record(..., true, null, ...) —— 调用成功，把用了哪个 provider、哪个 model、花了多少 token、耗时多久记一笔，success 记 true 。
- catch (RuntimeException e) { audit.record(..., false, e.getMessage(), ...); throw e; } —— 调用失败（超时、限流、模型报错）同样要记一笔，success 记 false 、把错误信息存进 error_message ，再把异常继续往上抛。这一步很容易漏：只记成功不记失败，一次真实事故在系统里就完全没留下痕迹。
