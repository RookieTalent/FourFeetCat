<!-- 来源讲义：第24节《Sandbox 实现与代码讲解》。本文件由该 PDF 逐页照搬整理为 Markdown，正文未改动内容，仅按原文版式重排；代码仅在 PDF 换行处还原为完整语句。 -->
...
```text
[FileTools.execute()]  ┐
[ShellTools.execute()] ├──►  [WhitelistSandbox.enforce(action)]
[HttpTools.execute()]  ┘                  │
                                          ▼
                          <命令/路径/域名在白名单里？>
                                 是 ──►  [继续执行 Tool]
                                 否 ──►  [抛 SandboxViolationException]

    不管放行还是拒绝，走的都是 ToolExecutor 原有的成功/失败审计路径，不需要新增审计代码
```
...
```text
«interface» Sandbox              SandboxAction
  enforce(SandboxAction)           type + target
        │ implements
        ▼
   WhitelistSandbox                 ActionType
    核心阶段唯一实现        FILE_ACCESS / SHELL_EXEC / HTTP_REQUEST
        │
        ├──► checkFilePath (private)      SandboxViolationException
        ├──► checkShellCommand (private)
        └──► checkHttpUrl (private)

    三个 check 方法都是 private，外部只看得到 enforce(SandboxAction) 这一个入口
```
...
[模型生成 shell 命令]
            │
            ▼
[ShellTools.execute()]  先调用 sandbox.enforce(cmd)
            │
            ▼
    <命令在 shell.allowed_commands 白名单里？>
      否 ──►  [抛 SandboxViolationException]  ┐
      是 ──►  [执行真正的 shell 命令]           ┘
                    │            │
                    └─────┬──────┘
                ...
                [ToolExecutor 捕获结果]
                          ▼
                [写入 tool_invocations]  (success=false/true, error_message)
                          ▼
                [结果追加回对话历史]  模型看到失败原因，下一轮可能调整策略
```
