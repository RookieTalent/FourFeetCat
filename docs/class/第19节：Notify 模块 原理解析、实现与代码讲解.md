<!-- 来源讲义：第19节《Notify 模块 原理解析、实现与代码讲解》。本文件由该 PDF 逐页照搬整理为 Markdown，正文未改动内容，仅按原文版式重排；代码仅在 PDF 换行处还原为完整语句。 -->
...
```text
外部：用户 / 业务系统 / 定时触发
CLI · REST API · AgentScheduler
入站 · ChannelAdapter
Agent 执行（ReActLoop）
想清楚要做什么，要不要主动往外说
出站 · NotifyChannelAdapter
外部：企业 IM 群 / Webhook
企业微信 · 飞书 · 钉钉

入站负责"怎么进来"，出站负责"怎么出去"——两个方向都要走 Adapter 插件模式，缺一个都不对称
```
...
```text
NotifyTools.notify(content, channel)
      ↓
NotifyChannelAdapter（接口）
表达"送到某个通知目标"，不携带具体渠道细节
      ↓
当前：WebhookNotifyAdapter          未来：企业微信/飞书/钉钉专用
通用 HTTP webhook，核心阶段唯一实现   新增实现类，接口不变（扩展阶段）

只要接口不变，webhook 换成专用 Adapter，NotifyTools 一行不用改
```
...
| --- | --- | --- |
| 企业微信 | https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=<KEY> | {"msgtype":"text","<br>{"content":"..."}} |
| 飞书 / Lark | https://open.feishu.cn/open-apis/bot/v2/hook/<TOKEN> | {"msg_type":"text",<br>{"text":"..."}} |
| 钉钉 | https://oapi.dingtalk.com/robot/send?access_token=<TOKEN> | {"msgtype":"text","<br>{"content":"..."}} |
...
- 企业微信要区分群机器人（webhook，本节这档）和应用消息（需要 corpid/corpsecret 换 AccessToken，属扩展阶段专用 Adapter 的活）。
