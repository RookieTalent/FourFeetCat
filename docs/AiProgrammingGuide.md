...
| acceptance criteria | 需求文档第 13 章 + 技术方案第 12 章的 3 个验收 Demo（每日天气、每日科技日报、每日 GitHub 日报） | 直接复用 |

已有文档的投入不浪费，Spec-Kit 只是把它们转换成 AI agent 能直接消费的格式。

> **关键注意**：技术方案是 `/speckit.plan` 的输入，所以 plan 里的模块结构必须跟技术方案第 10 章的 14 个模块一致，喂文档时确保用的是最新版技术方案，否则生成的 plan 会按错误的模块数拆分。另注意技术方案第 13 章（4 周节奏）里"搭 9 个模块骨架"说的是初始骨架，模块清单以第 10 章为准。
...
| # | 原则 | 说明 |
|---|------|------|
| 1 | JDK 21 + Spring Boot 3.x 单体应用 | Maven 多模块（14 个，技术方案第 10 章），单二进制部署 |
| 2 | 五大核心能力优先 | 核心阶段交付运行时内核，企业级治理层放扩展阶段 |
| 3 | 自实现 ReAct loop + **Spring AI 只用一半** | 不直接用 Spring AI 的 Agent 抽象；只用 Provider 抽象、协议转换、@Tool schema 生成，**禁用自动 tool 执行**，tool 调度完全由 `ReActLoop` + `ToolExecutor` 控制；provider name 到 `ChatModel` 用**显式映射**，不靠类型扫描。**最容易被写错的一条** |
| 4 | **一个目录 = 一个 Agent，且不是 Tool** | `AGENT.md` 正文由 `ContextLoader` 注入 system prompt（与 Bootstrap 文件同层）；Agent 目录归 core 的 `AgentLoader`/`ContextLoader`，不进 `ToolRegistry`；子资源经底座既有 `read_file`/`shell` 按需取用，不新造机制；Skill 用 `skills/` 相对软连接绑定（唯一绑定真相源），不写 frontmatter `skills:` 字段 |
| 5 | 接口先行 | `Sandbox`、`NotifyChannelAdapter`、`LongTermMemoryStore`、`InboundChannelAdapter` 等抽象接口不携带实现细节；扩展只新增实现类，不改接口、不改调用方 |
| 6 | Plugin Tool 三档接入 | 主推 AGENT.md 目录 + 复用 MCP 的零代码方式 |
| 7 | SQLite + Memory 三档后端 + 审计 day one | 长期记忆统一走 `LongTermMemoryStore` 接口墙，三档后端（Markdown 默认 / SQLite / Mem0）靠 `memory.backend` 一行切换；`tool_invocations` 和 `llm_calls` **核心阶段就写入落库**；凭证走 `${ENV_VAR}` 占位不落地；不用已废弃的 `SecurityManager` |
| 8 | 无状态实例，状态外置 | 单机先行、分阶段克制，为分布式预留；每次架构升级用真实使用数据证明必要性 |
| 9 | 每个 user story 完成后有可演示 Demo | 优先级是跑通而非完美 |
...
- 14 个 Maven 模块的职责（对照技术方案第 10 章；骨架先立 9 个基础模块，其余 5 个随演进增补）
...
| Task 类别 | 主要内容 |
|----------|---------|
| `MemoryService` 门面类 | 三层统一门面，内部把会话记忆委托给 `SessionManager`、长期记忆委托给 `LongTermMemoryStore` |
| `LongTermMemoryStore` 接口 + 三档后端 | `append(content, scope)`、`load`、`recallByKeyword`、`capabilities()`、`archivalEntries()`；核心/归档分区语义必选；三档实现 `MarkdownMemoryStore`（默认）/ `SqliteMemoryStore` / `Mem0MemoryStore`，靠 `memory.backend` 一行切换 |
| `MemoryTools` 类 | `save_memory` + `recall_memory` 两个内置 Tool，用 `@Tool` 注解，`scope` 参数显式指定核心/归档 |
| `PromptBuilder` 集成类 | 在 `PromptBuilder` 里通过 `MemoryService` 注入记忆，确保不破坏 US-2 跑通的 ReAct 循环 |
| `MEMORY.md` 文件管理类 | 文件位置、`## 核心记忆` / `## 归档记忆` 分区约定、截断只作用归档区 |
...
- **`WhitelistSandbox` 完整版**（从 US-2 的 HTTP 单路扩展到完整四路：文件读/写路径白名单 + Shell 命令白名单 + HTTP 域名白名单 + SMTP 端点白名单，建议拆 3~4 个子 task）
- **`NotifyTools` 出站通知**（`NotifyChannelAdapter` 接口 + `WebhookNotifyAdapter`，通知渠道走 SQLite `notify_channels` 全局注册表，发送前过 `Sandbox.enforce(HTTP_REQUEST, ...)` 域名白名单）
...
| 定时任务类 | `AgentScheduler`（第三触发源）：`ThreadPoolTaskScheduler` + `CronTrigger` 动态注册，Profile `schedules` 字段驱动；`ProfileRegistry` / `AgentScheduler` 的运行时注册方法在此阶段立好；进程内 `ReentrantLock` 防同一任务重叠执行 |
...
**最终验收**：三个 Demo 转钟推 + Web Service 联动

- 三个 Demo（每日天气、每日科技日报、每日 GitHub 日报）由 `AgentScheduler` 到点自动跑通，全程不需要人工触发；涉外调用都过 Sandbox 白名单且写入 `tool_invocations`；`GET /api/v1/sessions/{id}` 查得到自动触发的完整对话
- Web Service 同步调用：外部系统 `POST /api/v1/sessions` 创建 Session，`POST /api/v1/sessions/{id}/messages` 发消息，`GET` 查历史，`DELETE` 归档，完整链路跑通
- 多端点联动：外部系统调 `GET /info` 查健康 + Provider 列表、`GET /profiles` 列可用 Agent、`GET /tools` 查可用 Tool、`POST /agents/{name}/invoke` 无状态调用 Agent、`GET /memory` 查长期记忆，5 类端点协同完成一次业务流程
...
| 维度 | 主体开发 | 增量开发 |
|------|---------|---------|
| 任务颗粒度 | 大（14 个模块分批建） | 小（加一个 Channel、补一个 Bug） |
...
每个 user story 完成后有可演示 Demo，最终对应需求文档第 13 章与技术方案第 12 章的 3 个验收 Demo（以人推形态阶段性验证、US-5 后转钟推）。
